...
# Función para publicar los datos en MQTT cada 5 minutos
async def publish_mqtt_data():
    while True:
        await asyncio.sleep(30)  # Esperar 300 segundos (5 minutos)
        if last_data:
            # Publicar cada dato por separado en una sola línea
            mqtt_message = f"Timestamp: {last_data['Timestamp']}, Temperature: {last_data['Temperature']}, Humidity: {last_data['Humidity']}"
            mqtt_client.publish(mqtt_topic, mqtt_message)
            print(f"Datos publicados en MQTT: {mqtt_message}")
    ...
    while True:  # Intentar reconectar indefinidamente en caso de error
        try:
            async with BleakClient(address) as client:
                ...
                await asyncio.gather(print_last_data(), publish_mqtt_data())
